...
def check_for_updates() -> Optional[int]:
    ...
    # installs: `check_via_pypi()` compares against VERSION, so a
    ...
    # up to 6h (see #34491). The local_head guard (Prax custom) makes every TUI
    # start show truth IMMEDIATELY after a local rebase/pull/update.
    now = time.time()
    try:
        if cache_file.exists():
            ...
            ver_match = cached.get("ver") == VERSION
            # cache invalid if local HEAD has moved (pull/rebase/update busts cache)
            local_head_match = (
                cached.get("local_head") == local_head
                if has_local_git
                else True
            )
            if cache_age_ok and embedded_rev_match and ver_match and local_head_match:
