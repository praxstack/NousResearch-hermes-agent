...
1. ~~**Hermes interactive wizard does not expose `vpc_endpoint_url`.**~~ **Resolved** in `feat/native-bedrock-provider-20260428` (commit follow-up to this handoff). `hermes setup` → Bedrock now prompts for `vpc_endpoint_url` after the cross-region / global / prompt-cache toggles in both `_model_flow_bedrock_api_key` and the full `_model_flow_bedrock` paths. Skip-on-blank, validate-on-non-empty (https/http + host), persisted under `bedrock.vpc_endpoint_url` only when set, defaults to the existing value on re-run. Tests: `tests/hermes_cli/test_bedrock_vpc_endpoint_url.py` (13 cases).
2. ~~**Hermes default `bedrock` config block omits `vpc_endpoint_url`** as a commented reference.~~ **Resolved** in `feat/native-bedrock-provider-20260428` (follow-up to this handoff). The git-tracked example at `cli-config.yaml.example` now ships a fully commented-out `# bedrock:` block whose VPC-endpoint subsection points at `vpc_endpoint_url` with a runnable example URL and the env-var fallback chain (`AWS_ENDPOINT_URL_BEDROCK_RUNTIME` → `AWS_ENDPOINT_URL`), so users grepping the example file or copy-pasting it find the knob. The `hermes setup` wizard (deferred item #1) covers the interactive discovery path; the adapter (`agent/bedrock_adapter.py` lines 682–690) covers the runtime resolution.
