...
def build_converse_kwargs(
    ...
    # ── Sampling-param guard (Prax 2026-05-08; mostly ABSORBED UPSTREAM 2026-06-14) ──
    # Upstream now gates temperature/topP behind `_forbids_sampling_params(model)`
    # (Opus 4.7+ → both dropped), which fully covers our original Opus-4.7 guard.
    # The ONLY case upstream does NOT handle is the Opus-4.6 / Sonnet-4.6 family
    # rejecting temperature+top_p set TOGETHER (upstream allows both for 4.6).
    # We keep that one additive guard. `_bedrock_forbids_temp_and_top_p_together`
    # returns True only for the 4.6 family, so this never touches 4.7+/legacy.
    if (
        not _forbids_sampling_params(model)
        and _bedrock_forbids_temp_and_top_p_together(wire_model)
        and temperature is not None
        and top_p is not None
    ):
        top_p = None  # keep temperature, drop top_p — 4.6 family rejects both together
