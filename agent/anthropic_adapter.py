...
def build_anthropic_bedrock_client(
    ...
    kwargs: Dict[str, Any] = {
        "aws_region": region,
        "timeout": timeout,
        # Attach the common Anthropic beta headers as client-level defaults.
        # ``context-1m-2025-08-07`` unlocks the 1M context window for Opus 4.6/4.7
        # on Bedrock. Without this, Bedrock caps these models at 200K.
        # _CONTEXT_1M_BETA is explicitly appended because upstream now excludes
        # it from _COMMON_BETAS (some native Anthropic subscriptions reject
        # it) — we need it here for Bedrock's 1M-context opt-in.
