    ...
    if pconfig.auth_type == "aws_sdk":
        if provider == "bedrock":
            try:
                ...
                if not final_model:
                    logger.debug(
                        "resolve_provider_client: bedrock auxiliary got empty "
                ...
                # Only overrides for fallback chains — primary calls still
                ...
                if is_anthropic_bedrock_model(final_model):
                    ...
                    # Non-Claude Bedrock models (Nova, Llama, gpt-oss, …) go
                    # through the Converse-API shim instead of the Anthropic SDK.
                    real_client = None
            except Exception as exc:
                logger.warning(
            ...
            if real_client is None:
                converse_client = BedrockAuxiliaryClient(region, final_model)
                logger.debug(
                    "resolve_provider_client: bedrock converse (%s, %s)",
                    final_model, region,
                )
                return (_to_async_client(converse_client, final_model,
                                         is_vision=is_vision)
                        if async_mode else (converse_client, final_model))
